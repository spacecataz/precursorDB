...
dx = pd.DataFrame(x_coords)
dy = pd.DataFrame(y_coords)
dz = pd.DataFrame(z_coords)
median_x, median_y, median_z = [], [], []

i=0
while i < len(x_coords[0]):
    median_x.append(np.nanmedian(dx[i]))
    i+=1

i=0
while i < len(y_coords[0]):
    median_y.append(np.nanmedian(dy[i]))
    i+=1

i=0
while i < len(z_coords[0]):
    median_z.append(np.nanmedian(dz[i]))
    i+=1
...
axs[0].plot(mags['time'], median_x, color = 'red', label = 'Median')
...
plt.savefig('precursorDB/docs/EventList/SEA/EventsSummary/'+fileName+'/median/'+fileName+'_x.png')
...
axs[0].plot(mags['time'], median_y, color = 'red', label = 'Median')
axs[0].set_ylabel(r'$B \ (nT)$')
axs[0].set_xlim((xlim_min,xlim_max))
axs[0].set_ylim((min(yvals),max(yvals)))
axs[0].legend()
...
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
axs[2].set_xlabel('UT (HH:MM)')
plt.savefig('precursorDB/docs/EventList/SEA/EventsSummary/'+fileName+'/median/'+fileName+'_y.png')
...
axs[0].plot(mags['time'], median_z, color = 'red', label = 'Median')
axs[0].set_ylabel(r'$B \ (nT)$')
axs[0].set_xlim((xlim_min,xlim_max))
axs[0].set_ylim((min(yvals),max(yvals)))
axs[0].legend()
...
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
axs[2].set_xlabel('UT (HH:MM)')
plt.savefig('precursorDB/docs/EventList/SEA/EventsSummary/'+fileName+'/median/'+fileName+'_z.png')
