...
def biot_savart(R, ky, kz, width, dX=0.1):
    """
...
def calc_K(IMFd, IMFu):
    ...
    """
    return 1E-9 * (IMFd - IMFu) / mu0  # Units: A/m


def gmp_timeseries(IMFdz, IMFuz, IMFdy, IMFuy, Vsw, dT=60, r0=200, w_csheet=128, midpoint_dX=0.1):
    """
    ...
    Ky = calc_K(IMFdz, IMFuz)  # (-5, 127)
    Kz = -calc_K(IMFdy, IMFuy) # Made negative because of the factor of -1 present in the Ampere's law equation for Kz
    print(f"Ky = {Ky:0.4f}")
    print(f"Kz = {Kz:0.4f}")
