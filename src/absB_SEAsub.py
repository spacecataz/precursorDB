...
for i, item in enumerate(absB):
	absB[i] = absB[i] - item[newind]

for i, item in enumerate(absIntB):
	absIntB[i] = absIntB[i] - absIntB[-1]
...
plt.savefig('precursorDB/docs/EventList/SEA/EventsSummary/'+str(onset.year)+f"{onset.month:02}"+f"{onset.day:02}"+'/absolute_value/SEA_subtraction/'+fileName+'.png')

#plt.savefig('precursorDB/docs/EventList/SEA/absolute_value/SEA_subtraction/'+str(onset.year)+f"{onset.month:02}"+f"{onset.day:02}"+'.png')