...
#shift integrator results
for i in absB:
	t0vals.append(i[newind])

avgB = np.nansum(t0vals) / len(t0vals)
diffB = avgB - absIntB[-1]

for i, item in enumerate(absIntB):
	absIntB[i] = absIntB[i] + diffB
...
plt.savefig('precursorDB/docs/EventList/SEA/EventsSummary/'+str(onset.year)+f"{onset.month:02}"+f"{onset.day:02}"+'/absolute_value/no_subtraction/'+fileName+'.png')
'''
#plot
fig = plt.figure()
for i, plot in enumerate(absB):
...
plot1a, = plt.plot(timelist, absIntB, color = 'blue')
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
plt.legend(handles = [plot1a], labels = ['Biot Savart Integrator Result'])
plt.title('|B| for the {} / {} / {} Event'.format(onset.year, onset.month, onset.day))
plt.ylabel(r'$B \ (nT)$')
plt.xlabel('UT (HH:MM)')
#plt.gca().set_facecolor('dimgray')
#plt.gca().grid(color = 'silver')

plt.savefig('precursorDB/docs/EventList/SEA/absolute_value/no_subtraction/'+str(onset.year)+f"{onset.month:02}"+f"{onset.day:02}"+'.png')
'''